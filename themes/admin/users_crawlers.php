...
<td class="text">
  ...
  <a href="<?php echo base_url('admin/users/delete_crawlers'); ?>" onclick="return confirm('İstatistikleri sıfırlamak istiyor musunuz?')">Arama Motoru İstatistiklerini Sıfırla</a>
  <table width="100%" border="0" cellpadding="3" cellspacing="1">
  <tbody><tr> 
    <td colspan="6" bgcolor="#58838b" class="tablehl">Siteyi daha önce taramış olan Arama Motorları:</td>
  </tr>
  
  <tr> 
    <td width="21%" valign="top" bgcolor="#dee6e8" class="text"><strong>Arama Motoru</strong></td>
    <td width="26%" valign="top" bgcolor="#dee6e8" class="text"><strong>SESSION ID</strong></td>
    <td width="16%" valign="top" bgcolor="#dee6e8" class="text"><strong>IP Adresi</strong></td>
    <td width="22%" valign="top" bgcolor="#dee6e8" class="text"><strong>Son Ziyaret Tarihi</strong></td>
    <td width="15%" valign="top" bgcolor="#dee6e8" class="text"><strong>Toplam Ziyaret</strong></td>
  </tr>
  <?php
  	if (count($crawlers) < 1)
	{
		echo '<tr> 
    <td colspan="6" bgcolor="#F86262" class="tablehl">Siteyi ziyaret etmiş arama motoru bulunmamaktadır!</td>
  </tr>';
	} else {
		foreach ($crawlers as $c)
		{
			echo '<tr> 
					<td valign="top" bgcolor="#dee6e8" class="text">' . $c['data']['name'] . ' v.' . $c['data']['version'] . '</td>
					<td valign="top" bgcolor="#dee6e8" class="text">' . $c['data']['sess_id'] . '</td>
					<td valign="top" bgcolor="#dee6e8" class="text">' . $c['data']['ip'] . '</td>
					<td valign="top" bgcolor="#dee6e8" class="text">' . date('d.m.Y H:i:s', $c['data']['time']) . '</td>
					<td valign="top" bgcolor="#dee6e8" class="text">' . $c['num_rows'] . ' kez</td>
				  </tr>';
		}
	}
  ?>
